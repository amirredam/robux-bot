...
async def show_packages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for key, pkg in PACKAGES.items():
        ...
        if discount > 0:
            ...
            price_text = f"{discounted:,} تومان 🎁{discount}٪"
        else:
...
async def package_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(
        f"✅ انتخاب کردی: *{pkg['robux']:,} روباکس*\n"
...
async def receive_receipt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    admin_text = (
        ...
        f"🎮 `{roblox_username}`\n💎 {pkg['robux']:,} روباکس\n"
        f"💰 {final_price:,} تومان" + (f" (تخفیف {discount}٪)" if discount > 0 else "") +
        "\n━━━━━━━━━━━━━━━"
    )
...
async def my_account(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"🏷 اسم: *{u['name']}*\n⭐ امتیاز: *{u['points']}*\n"
        f"📦 سفارشات: *{u['orders']}*\n"
        f"💰 مجموع خرید: *{u['total_spent']:,} تومان*\n"
        f"👥 دعوت‌شدگان: *{u['referrals']} نفر*\n"
        f"📅 عضویت: *{u['joined']}*\n"
        f"🎖 سطح: *{level}*"
    )
    if u["discount"] > 0:
        text += f"\n🎁 تخفیف فعال: *{u['discount']}٪*"
    await update.message.reply_text(text, parse_mode="Markdown")

async def track_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("📦 شماره سفارش:\n`/order شماره_سفارش`", parse_mode="Markdown")
        return
...
async def use_discount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if code not in discounts or discounts[code]["used"] >= discounts[code]["max_use"]:
        await update.message.reply_text("❌ کد تخفیف نامعتبر یا تموم شده!")
        return
    register_user(update.effective_user)
    users[uid]["discount"] = discounts[code]["percent"]
    discounts[code]["used"] += 1
    await update.message.reply_text(
        f"✅ *تخفیف {discounts[code]['percent']}٪ فعال شد!* 🎁",
        parse_mode="Markdown"
    )
...
async def referral(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"لینک دعوت:\n`{ref_link}`\n\n"
        f"👥 دعوت‌شدگان: *{u['referrals']} نفر*\n"
        f"⭐ امتیاز کسب‌شده: *{u['referrals'] * 50}*\n\n"
        f"🎁 دوستت {REFERRAL_DISCOUNT}٪ تخفیف میگیره\nتو ۵۰ امتیاز میگیری!",
        parse_mode="Markdown"
    )
...
async def block_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if str(update.effective_user.id) != ADMIN_CHAT_ID:
        return
    if not context.args:
        await update.message.reply_text("فرمت: `/block آیدی`", parse_mode="Markdown")
        return
    tid = context.args[0]
    if tid not in users:
    ...
    await update.message.reply_text(f"✅ کاربر `{tid}` بلاک شد!", parse_mode="Markdown")
...
async def unblock_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args and context.args[0] in users:
        ...
        await update.message.reply_text(f"✅ آنبلاک شد!", parse_mode="Markdown")
...
async def add_discount_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 3:
        await update.message.reply_text("فرمت: `/addcode کد درصد تعداد`\nمثال: `/addcode RUBAX20 20 100`", parse_mode="Markdown")
        return
    ...
    await update.message.reply_text(f"✅ کد `{code}` با {percent}٪ برای {max_use} بار ساخته شد!", parse_mode="Markdown")
